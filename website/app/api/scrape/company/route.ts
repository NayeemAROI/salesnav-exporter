...
import { scanCompanies } from "@/lib/company-scanner";
import { acquireScrapeJob, ApiInputError, apiError, assertBodySize, linkedinUrl, safeError, sanitizeLinkedInCookies, serverProxy, sseHeaders } from "@/lib/api-security";
  ...
  try {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        try {
          for await (const progress of scanCompanies(urls, cookies, { proxy })) {
