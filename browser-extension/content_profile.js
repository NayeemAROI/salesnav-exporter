...
if (!window._salesNavProfileScannerInstalled) {
  ...
  async function extractProfileMain() {
    ...
    try {
      ...
      // Followers are reported exactly (unlike connections, capped at "500+"), so capture
      // them separately and fold into connectionCountInt — drives the "5000+ = active" rule.
      const followerMatch = allText.match(/([\d,]+)\s+followers/i);
      if (followerMatch) {
        const followerInt = parseInt(followerMatch[1].replace(/,/g, ''), 10) || 0;
        if (followerInt > connectionCountInt) connectionCountInt = followerInt;
      }
  ...
  function formatTimeAgo(date) {
    ...
    if (diffDays >= 7) {
      ...
      if (diffWeeks < 5) {
        ...
        if (diffMonths < 12) {
          ...
          value = Math.floor(diffMonths / 12);
          unit = 'year';
        }
      }
    } else if (diffDays >= 1) {
    ...
    const formatted = rtf.format(-value, unit).replace(' ago', '');
  ...
  async function extractProfileActivity() {
    ...
    try {
      ...
      // Kept in sync with background.js timeStringToDays: unknown / N/A => Infinity (treated as very old).
      function timeStringToDays(t) {
        ...
        return Infinity; // unknown = very old
      }
