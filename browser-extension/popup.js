...
async function copyData(rows, mode) {
  ...
  const res = await send({ type: 'CONVERT_TO_CSV', rows, mode });
...
document.getElementById('startScanner').onclick = async () => {
  ...
  // Accept lines with or without a scheme (linkedin.com/in/..., www...., https://...)
  const lines = text.split(/\r?\n/).map(u => u.trim()).filter(Boolean);
  const norm = (u) => /^https?:\/\//i.test(u) ? u : 'https://' + u.replace(/^\/+/, '');

  const invalidUrls = lines.filter(u => !u.includes('linkedin.com/in/'));
  ...
  if (results.length > 0 && previewBody && previewTable) {
    ...
    for (const r of show) {
      ...
      tr.innerHTML = `
        ...
        <td style="max-width:120px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${escHtml(r.website)}</td>
        <td>${escHtml(r.industry)}</td>
        <td>${escHtml(r.companySize)}</td>
        <td>${escHtml(r.headquarters)}</td>
        <td>${escHtml(r.founded)}</td>
      `;
